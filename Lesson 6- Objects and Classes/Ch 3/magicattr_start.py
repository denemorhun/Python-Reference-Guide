...
class Book:
    ...
    def __str__(self):
        return f"{self.title} by {self.author}, costs {round(self.price)}"
    ...
    def __setattr__(self, name, value):
        if name == "price":
            if type(value) is not float:
                raise ValueError("Input is not float")
...
b1.price = 40.99
# print(b1, "with discounted price", b1.price)

print(round(b1.price, 2))
